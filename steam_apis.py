...
def load_esrb_datasets():
    ...
    train_esrb_df = pd.read_csv('src/data/train_esrb.csv')
    test_esrb_df = pd.read_csv('src/data/test_esrb.csv')
...
def save_all_apps_to_csv(all_apps):
    ...
    all_apps_df.to_csv('data/all_apps.csv', index=False)
...
def map_titles_to_app_ids(combined_esrb_df, title_to_app_id):
    ...
    title_appid_pd = pd.DataFrame(columns=['title', 'app_id', 'esrb_rating'])
    title_appid_pd['title'] = combined_esrb_df['title']
    title_appid_pd['app_id'] = title_appid_pd['title'].map(title_to_app_id).astype(pd.Int64Dtype()).values
    print(title_appid_pd.head()
          )
    return title_appid_pd
